...
import { CycleEndRow, CycleStartRow, RestStartRow } from "./TimelineEventRows";
...
export type TimelineEvent =
  ...
  | { type: "rest-start"; date: string; timestamp: number }
  | { type: "rest-end"; date: string; timestamp: number };
...
interface SessionHistoryTableProps {
  ...
  onShowRestPeriodsChange: (show: boolean) => void;
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-end gap-4">
        ...
        <label className="flex cursor-pointer items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          <input
            type="checkbox"
            checked={showRestPeriods}
            onChange={(e) => onShowRestPeriodsChange(e.target.checked)}
            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800"
          />
          <span>😴 {t("exercise.filters.showRests")}</span>
        </label>
      </div>
            ...
            <tbody>
              {timeline.map((event) => {
                ...
                if (event.type === "rest-start") {
                  return <RestStartRow key={`rest-start-${event.date}`} date={event.date} />;
                }
                return null;
              })}
            </tbody>
