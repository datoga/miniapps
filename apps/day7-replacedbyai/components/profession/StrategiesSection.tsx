          ...
          {profession.adaptationStrategies.map((strategy, index) => {
            ...
            return (
              ...
              >
                {/* Marker dot */}
                <div
                  className={`relative z-10 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br ${config.gradient} flex items-center justify-center text-xl sm:text-2xl shadow-lg transition-all duration-300 ${getMarkerOpacityClass(isActive, isPast, config.ring)}`}
                >
                  {config.icon}
                </div>
                {/* Timeframe label */}
